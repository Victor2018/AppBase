...
class FlowLayout: ViewGroup,View.OnClickListener {
    ...
    var mOnLabelClickListener: ((View?,String?,Int) -> Unit)? = null

    var isSingleCheck = true//单选
    var checkPosition: Int = -1
    var checkMap = LinkedHashMap<Int,Int>()
        ...
        childs?.let {
            ...
            for (i in 0 until childs.size) {
                if (childs[i].tag == null) {
                    ...
                    if (childs[i].tag is Boolean) {
                        val clickable = childs[i].tag as? Boolean
                        if (clickable == true) {
                            childs[i].tag = i
                        } else {
                            childs[i].tag = -1
    ...
    override fun onClick(v: View?) {
        if (v is TextView) {
            ...
            if (isSingleCheck) {
                ...
                requestLayout()
                mOnLabelClickListener?.invoke(v,v?.text.toString(),v?.tag.toString().toInt())
            } else {
                if (isLabelChecked(position)) {
                    checkMap.remove(position)
                } else {
                    checkMap[position] = position
                }
                requestLayout()

                if (checkMap.size == childCount) {//全选
                    mOnLabelClickListener?.invoke(v,null,-1)
                } else {
                    mOnLabelClickListener?.invoke(v,v?.text.toString(),v?.tag.toString().toInt())
                }
            }
    ...
    fun setTextViewStyle (textView: TextView) {
        ...
        val index = textView.tag.toString().toInt()
        if (isLabelChecked(index)) {
    ...
    fun isLabelChecked(labelIndex: Int): Boolean {
        if (labelIndex < 0) return false
        if (isSingleCheck) {
            return checkPosition == labelIndex
        } else {
            return checkMap[labelIndex] != null
        }
    }
