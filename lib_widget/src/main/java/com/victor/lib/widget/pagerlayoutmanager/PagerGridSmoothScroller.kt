 ...
 * -----------------------------------------------------------------
 * File: PagerGridSmoothScroller
 * Author: Victor
 * Date: 2022/10/9 12:24
 * Description: 
 * -----------------------------------------------------------------
 */

class PagerGridSmoothScroller(var recyclerView: RecyclerView?,
                              var layoutManager: PagerGridLayoutManager) :
    LinearSmoothScroller(recyclerView?.context) {

    private val TAG = "PagerGridSmoothScroller"
    ...
    companion object {
         ...
         */
        const val MAX_SCROLL_ON_FLING_DURATION = 500 //ms
    ...
    override fun onTargetFound(targetView: View, state: RecyclerView.State, action: Action) {
        ...
        if (layoutManager is PagerGridLayoutManager) {
            ...
            Log.e(TAG, "onTargetFound-targetPosition:$targetPosition, " +
                    "dx:$dx,dy:$dy,time:$time,isLayoutToEnd:$isLayoutToEnd," +
                    "snapRect:$snapRect,targetRect:$targetRect"
            )
            if (time > 0) {
    ...
    override fun calculateSpeedPerPixel(displayMetrics: DisplayMetrics): Float {
        val speed = layoutManager.getMillisecondPreInch() / displayMetrics.densityDpi
        Log.e(TAG, "calculateSpeedPerPixel-speed: $speed")
        return speed
    }
    ...
    override fun calculateTimeForScrolling(dx: Int): Int {
        val time = Math.min(
            layoutManager.getMaxScrollOnFlingDuration(),
            super.calculateTimeForScrolling(dx)
        )
        Log.e(TAG, "calculateTimeForScrolling-time: $time")
        return time
    }
