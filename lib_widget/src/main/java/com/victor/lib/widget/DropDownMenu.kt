...
import com.victor.lib.widget.util.ScreenUtils
...
class DropDownMenu: LinearLayout {
    ...
    var mOnDropDownMenuClickListener: ((Int)->Unit)? = null
    var mOnDropDownMenuShowListener: ((Boolean)->Unit)? = null
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(context, attrs, defStyleAttr) {
        ...
        underLine.layoutParams = LayoutParams(LayoutParams.MATCH_PARENT, dpTpPx(1.0f))
    ...
    fun setDropDownMenu(tabTexts: List<String>, popupViews: List<View>, contentView: View?) {
        ...
        if (menuHeighPercent > 0) {
            popupMenuViews!!.layoutParams = FrameLayout.LayoutParams(
                LayoutParams.MATCH_PARENT, (
                        ScreenUtils.getHeight(context) * menuHeighPercent).toInt())
        } else {
            contentView?.post {
                popupMenuViews?.layoutParams = FrameLayout.LayoutParams(
                    LayoutParams.MATCH_PARENT,
        ...
        for (i in popupViews.indices) {
            popupViews[i].layoutParams = ViewGroup.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT)
    ...
    private fun addTab(tabTexts: List<String>, i: Int) {
        ...
        var tabParams = LayoutParams(0, LayoutParams.WRAP_CONTENT, 1f)

        if (!isFillWidth) {
            tabParams = LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT)
        }
        ...
        tab.layoutParams = LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT)
        if (isTextViewFillWidth) {
            tab.layoutParams = LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT)
        }
        tab.setTextColor(textUnselectedColor)
        tab.setCompoundDrawablesWithIntrinsicBounds(null, null, resources.getDrawable(menuUnselectedIcon), null)
        tab.compoundDrawablePadding = ResUtils.getDimenFloatPixRes(context,com.victor.screen.match.library.R.dimen.dp_12).toInt()
        tab.text = tabTexts[i]
        ...
        if (i < tabTexts.size - 1) {
            ...
            view.layoutParams = LayoutParams(dpTpPx(0.5f), LayoutParams.MATCH_PARENT)
    ...
    fun closeMenu() {
        ...
        mOnDropDownMenuShowListener?.invoke(false)
    }
    ...
    private fun switchMenu(target: View) {
            ...
            if (target === tabMenuView!!.getChildAt(i)) {
                if (current_tab_position == i) {
                    ...
                    if (current_tab_position == -1) {
                        ...
                        popupMenuViews!!.getChildAt(i / 2).visibility = VISIBLE
                    } else {
                        popupMenuViews!!.getChildAt(i / 2).visibility = VISIBLE
                    }
        ...
        Log.e(TAG, "switchMenu-position = " + position)
        mOnDropDownMenuClickListener?.invoke(position)
    }
