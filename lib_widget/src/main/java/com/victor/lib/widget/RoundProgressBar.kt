 ...
 * -----------------------------------------------------------------
 * File: RoundProgressBar
 * Author: Victor
 * Date: 2026/1/25 13:50
 * Description: 
 * -----------------------------------------------------------------
 */
...
class RoundProgressBar: View {
    ...
    private fun initAttr(context: Context,attrs: AttributeSet?) {
        ...
        try {
            mBackgroundColor =
                a.getColor(
                    R.styleable.RoundProgressBar_rpb_background_color,
                    context.getColor(R.color.color_333333)
                )
            mPrimaryProgressColor =
                a.getColor(
                    R.styleable.RoundProgressBar_rpb_progress_color,
                    context.getColor(R.color.colorPrimary)
                )
            mZeroProgressEnabled =
                a.getBoolean(R.styleable.RoundProgressBar_rpb_zero_progress_enabled, true)
            mMax = a.getInt(R.styleable.RoundProgressBar_rpb_max, 100)
            mProgress = a.getInt(R.styleable.RoundProgressBar_rpb_progress, 0)
            mAnimationProgress = mProgress.toFloat()
            mStrokeWidth = a.getDimensionPixelSize(R.styleable.RoundProgressBar_rpb_stroke_width, 8)
            mBackgroundStrokeWidth =
                a.getDimensionPixelSize(R.styleable.RoundProgressBar_rpb_background_stroke_width, 4)
            mEndCapSize =
                a.getDimensionPixelSize(R.styleable.RoundProgressBar_rpb_end_cap_size, 2 * mStrokeWidth)
            mEndCapVisible = a.getBoolean(R.styleable.RoundProgressBar_rpb_end_cap_visible, false)
            mAnimationDuration = a.getInteger(R.styleable.RoundProgressBar_rpb_animation_duration, 1000)
            mProgressBarStyle = a.getInteger(R.styleable.RoundProgressBar_rpb_style, 0)
            mBackgroundStyle = a.getInteger(R.styleable.RoundProgressBar_rpb_background_style, 0)
        } finally {
            a.recycle()
        }
    ...
    inner class RoundProgressBarAnimation(
